...
# I want the total daily volume and avg volume and avg speed/station #
data_stid <- data |>
...
# lubridate will round up to the nearest hour/day/etc ----
...
# interactive version of the figure above, so you can see missing dates, etc.
library(plotly)
ggplotly(daily_volume_fig)

# base R function to show how many stations there are:
length(unique(daily_data$stationid))

stids <- unique(daily_data$stationid)

# for March, you expect 31 days and 23 stations (above function answer)
...
# exports a csv in your folder for you to open with excel
write.csv(data_with_gaps, "data/data_with_gaps.csv", row.names = F)

# saves the file with the class structures of the file and compresses file
